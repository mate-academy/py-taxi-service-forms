...
def license_number_validator(value):
    if not value[:DRIVER_LICENSE_NUMBER_LETTERS_IN_UPPER].isupper():
        raise ValidationError(f"First {DRIVER_LICENSE_NUMBER_LETTERS_IN_UPPER} letters must be in uppercase")
    if not len(value) == DRIVER_LICENSE_NUMBER_MAX_LENGHT:
        raise ValidationError(f"License number must contain {DRIVER_LICENSE_NUMBER_MAX_LENGHT} letters")
    if not value[-DRIVER_LICENSE_NUMBER_NUMBER_OF_NUM_LETTERS:].isnumeric():
        raise ValidationError(f"Last letters {DRIVER_LICENSE_NUMBER_NUMBER_OF_NUM_LETTERS} must be digits")
